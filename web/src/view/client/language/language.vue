<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="语言代码" prop="code" width="100" />
        <el-table-column align="left" label="语言名称" prop="name" width="150" />
        <el-table-column align="left" label="原生名称" prop="nativeName" width="150" />
        <el-table-column align="left" label="排序" prop="sort" width="80" />
        <el-table-column align="left" label="默认" width="80">
...
const getTableData = async() => {
  const table = await getLanguageList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' })
    .then(async() => {
      ...
      for (const id of IDs) { await deleteLanguage({ ID: id }) }
...
const enterDialog = async () => {
  ...
  elFormRef.value?.validate(async (valid) => {
    ...
    let res
    switch (type.value) {
      case 'create': res = await createLanguage(formData.value); break
      case 'update': res = await updateLanguage(formData.value); break
      default: res = await createLanguage(formData.value); break
    }
