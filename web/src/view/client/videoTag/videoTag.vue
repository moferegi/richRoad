  ...
  <div>
    ...
    <div class="gva-table-box">
        <div class="gva-btn-list">
            ...
            <ExportTemplate  template-id="client_VideoTag" />
            <ExportExcel  template-id="client_VideoTag" filterDeleted/>
            <ImportExcel  template-id="client_VideoTag" @on-success="getTableData" />
        </div>
        <el-table
        ref="multipleTable"
        style="width: 100%"
        ...
        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            ...
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateVideoTagFunc(scope.row)">编辑</el-button>
    ...
    <el-drawer destroy-on-close :size="appStore.drawerSize" v-model="dialogFormVisible" :show-close="false" :before-close="closeDialog">
       <template #header>
              <div class="flex justify-between items-center">
                <span class="text-lg">{{type==='create'?'新增视频标签':'编辑视频标签'}}</span>
                <div>
    ...
    <el-drawer destroy-on-close :size="appStore.drawerSize" v-model="detailShow" :show-close="true" :before-close="closeDetailShow" title="查看视频标签">
...
const getTableData = async() => {
  const table = await getVideoTagList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteVideoTagByIds({ ids: ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
const deleteVideoTagFunc = async(row) => {
    const res = await deleteVideoTag({ id: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const updateVideoTagFunc = async(row) => {
    const res = await findVideoTag({ id: row.ID })
...
const enterDialog = async() => {
    ...
    elFormRef.value?.validate(async(valid) => {
        ...
        if (type.value === 'create') {
            res = await createVideoTag(formData.value)
        } else {
            res = await updateVideoTag(formData.value)
        }
        if (res.code === 0) {
            ElMessage({
                type: 'success',
