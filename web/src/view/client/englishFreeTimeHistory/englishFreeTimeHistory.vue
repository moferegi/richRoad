  ...
  <div>
    ...
    <div class="gva-table-box">
      ...
      >
        ...
        <el-table-column align="left" label="时长变化" width="120">
          <template #default="scope">
            <span :class="scope.row.changeType === 'increase' ? 'text-success' : 'text-danger'" class="minute-change">
              {{ scope.row.changeType === 'increase' ? '+' : '' }}{{ scope.row.minuteChange }}分钟
            </span>
          </template>
        </el-table-column>
        ...
        <el-table-column align="left" label="当前时长" width="120">
          <template #default="scope">
            <span>{{ scope.row.currentFreeMinutes }}分钟</span>
          </template>
        </el-table-column>
        <el-table-column align="left" label="关联单集ID" prop="relatedEpisodeId" width="110" />
...
<script setup>
import { getAdminFreeTimeRecordList, getAdminUserList } from '@/api/client/englishLearningAdmin'
import { formatDate } from '@/utils/format'
import { ref } from 'vue'

defineOptions({
  name: 'EnglishFreeTimeHistory'
})
...
const getTableData = async () => {
  ...
  const res = await getAdminFreeTimeRecordList(params)
  if (res.code === 0) {
