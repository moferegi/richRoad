  ...
  <div>
    ...
    <div class="gva-table-box">
        <div class="gva-btn-list">
            ...
            <ExportTemplate  template-id="shop_Kefu" />
            <ExportExcel  template-id="shop_Kefu" filterDeleted/>
            <ImportExcel  template-id="shop_Kefu" @on-success="getTableData" />
        </div>
        <el-table
        ref="multipleTable"
        style="width: 100%"
    ...
    <template #default="scope">
      <el-image preview-teleported style="width: 100px; height: 100px" :src="scope.row.externalAvatar || getUrl(scope.row.avatar)" fit="cover"/>
        ...
        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            ...
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateKefuFunc(scope.row)">编辑</el-button>
...
const getTableData = async() => {
  const table = await getKefuList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteKefuByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateKefuFunc = async(row) => {
    const res = await findKefu({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteKefuFunc = async (row) => {
    const res = await deleteKefu({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
