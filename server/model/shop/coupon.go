...
type Coupon struct {
	...
	Type        string     `json:"type" form:"type" gorm:"comment:优惠券类型;column:type;type:enum('product','discount','no_threshold');" binding:"required"` //类型
	Discount    *float64   `json:"discount" form:"discount" gorm:"comment:折扣金额;column:discount;" binding:"required"`                                     //折扣
	MinSpend    *float64   `json:"minSpend" form:"minSpend" gorm:"comment:最低消费金额;column:min_spend;" binding:"required"`                                  //最低消费
	...
	Claimed     *int       `json:"claimed" form:"claimed" gorm:"comment:已领取数量;column:claimed;" binding:"required"`                                       //已领取
