...
5. **UniApp 多语言强制要求**:
   - Uni 页面所有用户可见文本（模板文案、按钮文案、空态、错误提示、placeholder、`uni.showToast`、`uni.showModal`、JS 里拼接文本）**必须**走 i18n，禁止新增硬编码中文。
   - 新增功能涉及后端接口返回字段且会展示给 Uni 用户时，**必须**提前定义多语言方案（i18n key 或多语言对象），不能仅返回单语字符串。
   - 对后端返回的多语言字段（对象或 JSON 字符串，如 `{mn:"",en:"",zh:""}`），展示前**必须**统一用 `localText(value, langStore.locale)`（或等价封装）解析，禁止 `String(value)` 直接渲染。
   - 系统配置文案（如试衣间公告 `announcement_content`）默认按多语言解析，并确保语言切换后内容同步变化。
   - 新增词条时，必须同步更新 `layout/client/language`（或项目实际 i18n 词条文件）并至少覆盖当前启用语种。
...
2. **数据格式**:
   ...
   - **⚠️ 多语言字段约定**：
      - 面向 Uni 用户展示的后端文本字段，推荐返回 i18n key 或多语言对象（如 `{zh,en,mn}`），避免只返回中文。
      - 当前端接收多语言对象或 JSON 字符串时，必须按当前语言解析后展示，禁止直接字符串化输出。
      - 接口改动影响文案时，前后端需同步更新 `layout/client/language`（或项目实际 i18n 文件）词条，避免缺词回退。
