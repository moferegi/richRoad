...
export default {
    ...
    methods: {
        /**
         * 获取折扣标签文本
         * @param {number} discount - 折扣值(0-10)
         * @returns {string} 折扣描述文本
         */
                ...
                for (let i = 0; i < pageSize; i++) {
                    ...
                    if (pageNo <= 3) { // 只模拟3页数据
                        pageData.push({
                            ...this.mockData[index],
                            // 添加页码标记，方便区分不同页的数据
                            name: `${this.mockData[index].name} - ${pageNo}-${i + 1}`,
                            // 确保不同页的图片URL不重复
                            image: `${this.mockData[index].image}&page=${pageNo}&item=${i}`
                        })
                    }
                }
