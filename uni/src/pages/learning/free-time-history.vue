  ...
  <view class="free-time-history-container">
    ...
    <view class="page-hero">
      ...
      <view class="hero-content">
        ...
        <text class="hero-title">{{ t('learningFreeTimeHistoryTitle') }}</text>
    ...
    <scroll-view class="list-scroll" scroll-y @scrolltolower="loadMore">
      ...
      <view v-for="(item, index) in recordList" :key="`${item.id}-${index}`" class="record-card">
        <view class="record-icon-wrap" :class="item.minuteChange >= 0 ? 'is-plus' : 'is-minus'">
          <text class="record-icon">{{ item.minuteChange >= 0 ? '+' : '−' }}</text>
        </view>
        <view class="record-content">
          <text class="record-reason">{{ getReasonText(item.reason) }}</text>
          <text class="record-time">{{ formatTime(item.createdAt) }}</text>
        </view>
        <view class="record-value-pill" :class="item.minuteChange >= 0 ? 'is-plus' : 'is-minus'">
          <text class="record-value-text">{{ item.minuteChange > 0 ? '+' : '' }}{{ item.minuteChange }} {{ t('profile.minutes') }}</text>
        </view>
      </view>
