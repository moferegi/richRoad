...
import { getLearningFreeTimeRecordList, getAsset } from '@/api/learning'
...
async function loadAsset() {
  ...
  try {
    ...
    if (res.code === 0) {
      currentFreeTime.value = res.data?.freeMinutes || res.data?.freeTime || 0
    }
  } catch (e) {
...
async function loadRecords(reset = false) {
  ...
  loading.value = true
  try {
    const res = await getLearningFreeTimeRecordList({
      page: page.value,
    ...
    console.warn('load free time records failed', e)
  } finally {
    loading.value = false
  }
}
...
function getType(item) {
  return item.type || item.typeName || item.reason || '时长变动'
}

// 获取变动值（分钟）
function getChange(item) {
  return item.minutes || item.change || item.amount || item.freeMinutes || 0
}
  ...
  <div class="record-page">
    <div class="record-content">
      ...
      <div class="page-header">
        <h1 class="page-title">{{ t('freeTimeHistory.title') }}</h1>
      </div>
      ...
      <div class="overview-card time-card">
        ...
        <div class="card-info">
          <div class="card-label">{{ t('freeTimeHistory.currentTime') }}</div>
          <div class="card-value">
            <span class="value-num">{{ loadingAsset ? '--' : formatDuration(currentFreeTime) }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="list-header">
        <span>{{ t('freeTimeHistory.time') }}</span>
        <span>{{ t('freeTimeHistory.type') }}</span>
        <span>{{ t('freeTimeHistory.change') }}</span>
      </div>
      ...
      <div class="record-list">
        ...
        >
          ...
          <div class="item-change" :class="{ positive: isIncrease(item), negative: !isIncrease(item) }">
            {{ isIncrease(item) ? '+' : '-' }}{{ formatDuration(getChange(item)) }}
          </div>
        </div>
...
.overview-card {
  ...
  &.time-card {
    background: linear-gradient(135deg, rgba(109, 91, 255, 0.1) 0%, rgba(155, 143, 255, 0.05) 100%);
